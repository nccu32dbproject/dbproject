...
def insert_item2(a,b,_cursor):
    command = "INSERT INTO Menu (Restaurant,Style) VALUES (\'%s\', \'%s\');" % (a,b)
    _cursor.execute(command)
...
#    data1 = load_data1()
    ...
    for items in data1:
        ...
        '''
    for items in data2:
        insert_item2(items[0],items[1],conn.cursor())
    conn.commit()
    conn.close()
