...
def load_data1(path='./rest.csv'):
    Rest = list()
    with open('./rest.csv','r') as f:
        for line in f.readlines()[1:]:
            Rest.append(line.rstrip().split(','))
    return Rest

def load_data2(path='./add.csv'):
    Add = list()
...
def insert_item1(a,b,c,d,e,f,g,h,_cursor):
    command = "INSERT INTO Restaurant (Name, Phone, Style, OT, Facebook, Seat, Payment, Line) \
            VALUES (\'%s\', \'%s\', %d, \'%s\', \'%s\', %d, %d, \'%s\');" % (a,b,c,d,e,f,g,h)
...
if __name__=="__main__":
    ...
    conn = connect_db()
    for items in data1:
        insert_item1(items[0],items[1],int(items[2]),items[3],items[4],int(items[5]),int(items[6]),items[7], conn.cursor())
